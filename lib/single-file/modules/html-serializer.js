			...
			chrome.storage.local.get('times', function (result) {
		      ...
		      //if(times == "first"){
					if(false){
						var serverURL;
				    var serverToken;
						chrome.storage.local.get('chromeToken', function (result) {
				        serverToken = result.chromeToken;
				      chrome.storage.local.get('serverURL', function (result) {
				        const serverURL = result.serverURL;
					  	  var http = new XMLHttpRequest();
					  	  var url = serverURL+"document/upload";
					  	  var formData = new FormData();
					  	  var content = docTypeString + (compressHTML ? serialize(doc.documentElement) : doc.documentElement.outerHTML)
			          ...
			          formData.append("theFile", blob, "InContext.html");
						...
						chrome.storage.local.set({'times': "second"});
					}else{
						var serverURL;
				    var serverToken;
						chrome.storage.local.get('chromeToken', function (result) {
						    ...
						    http.onreadystatechange = function() {
						        if(http.readyState == 4 && http.status == 200) {
										...
										if(http.readyState == 4 && http.status == 403) {
							  	  		...
							  	  		alert("Failed to send page to Linpogort InContext server. This page doesn't contain a valid LRM key. You could turn off Lingoport InContext extension Auto-capture option, or close this tab. ");
								    }
								    ...
								    if(http.readyState == 4 && http.status == 402) {
									    	console.log("402");
									     	alert("Failed to send page to Linpogort InContext server. The chrome token of Lingoport InContext extension is invalid.");
						  	  	}
					  	  }
					  	  http.send(formData);
							});
						});
						chrome.storage.local.set({'times': "first"});
					}
					});
