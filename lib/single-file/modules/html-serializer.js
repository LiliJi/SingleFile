...
this.serializer = this.serializer || (() => {
	...
	return {
		process(doc, compressHTML) {
			...
			formData.append("tmp_file_name", docTypeString + (compressHTML ? serialize(doc.documentElement) : doc.documentElement.outerHTML));
      var xhr = new XMLHttpRequest();
			$.ajax({
				type:'POST',
				url:'http://ec2-3-82-38-59.compute-1.amazonaws.com:8081/incontext-server/document/upload',
				data:formData,
				datatype: "html",
				processData:false,
				traditional: true,
				...
				beforeSend: function (xhr){
					xhr.setRequestHeader('Authorization',
					   'Basic ' + btoa('Dev' + ':' + 'W61y21WzEDJ6K5i5Lv67OOSiQD3A6w6h'));
				}
			}).done(function(data, textStatus, jqXHR){
				alert("Sent successfully.")
			//	sendResponse({statusok: 'true', statusmsg: textStatus, retranid: retranId});
			...
			//	sendResponse({statusok: 'false', statusmsg: errorThrown, retranid: retranId});
			});
